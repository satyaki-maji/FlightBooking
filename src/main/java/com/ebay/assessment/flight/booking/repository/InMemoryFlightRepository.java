 ...
 *
 * <p>Each seeded flight is given a fresh {@link ConcurrentLinkedQueue}
 * of 100 unique seat numbers (rows 1–25, columns A–D) via
 * {@link #buildSeatQueue()}, replacing the old integer
 * {@code availableSeats} field.</p>
 *
...
public class InMemoryFlightRepository implements FlightRepository {
    ...
    /**
     * Builds a fresh {@link ConcurrentLinkedQueue} pre-filled with
     * exactly 100 unique, sequential seat numbers.
     *
     * <p>Seats are generated by iterating rows {@code 1–25} (outer loop)
     * and columns {@code A–D} (inner loop), producing the ordered
     * sequence {@code 1A, 1B, 1C, 1D, 2A, …, 25C, 25D}
     * (25 rows × 4 columns = 100 seats).</p>
     *
     * <p>A new independent queue is returned on every call so that each
     * seeded {@link Flight} owns its own private pool of seats.</p>
     *
     * @return a non-empty {@link ConcurrentLinkedQueue} of 100 seat labels
     */
    private ConcurrentLinkedQueue<String> buildSeatQueue() {
        ConcurrentLinkedQueue<String> queue = new ConcurrentLinkedQueue<>();
        char[] columns = {'A', 'B', 'C', 'D'}; // 4 columns per row
        for (int row = 1; row <= 25; row++) {   // 25 rows  →  25 × 4 = 100 seats
            for (char col : columns) {
                queue.add(row + String.valueOf(col));
            }
        }
        return queue;
    }
